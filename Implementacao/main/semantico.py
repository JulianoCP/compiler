...
def createMainRoot():
    ...
    global tempNewRoot
    tempNewRoot = programa
    return programa

def readAllNodesNewRoot(node):
    if node.parent != None:
        print(node.name,node.parent.name)
    for i in node.children:
        readAllNodesNewRoot(i)
...
def getLastChildren(node):
    if len(node.children) >= 1:
        ...
        if node.name == "vazio":
            ...
            addParentInTreeGenerate(new_node)
        else:
...
def chooseNewParent(name):
    cases = {
        "lista_declaracoes": [0,True],
        ...
        "lista_variaveis":[1,True],
        "escreva":[1,True],
        "indice":[1,True],
        "parametro": [1,True],
        "lista_parametros":[1,True],
        "retorna":[1,True],
        "corpo":[1,True],
        "chamada_funcao":[1,True],
        "repita":[1,True],
        "leia":[1,True],
        "se":[1,True],
    }
    return cases.get(name, [-1,False])

def splitTree(node,parent):
    if len(node.children) > 1:
        count = 0
        values = chooseNewParent(node.name)
        select = values[0]
        verifyToChangeParent = values[1]
        if select == -1:
            select = math.ceil(len(node.children)/2) -1
        if verifyToChangeParent == True:
            new_node = MyNode(name=node.name)
            new_node.parent = parent
            parent = new_node
            addParentInTreeGenerate(new_node)
        else:
            getLastChildren(node.children[select])
            tempParentGenerateTree[0].parent = parent
            parent = tempParentGenerateTree[0]
        for i in node.children:
            if verifyToChangeParent == False:
                if count != select:
                    splitTree(i, parent)
            else:
                splitTree(i,parent)
        ...
        splitTree(node.children[0], parent)
    else:
